    ...
    <AccountLayout>
      <div>
        <h1 className="text-xl font-extrabold text-gray-900 mb-5">
          Ma Wishlist <span className="text-gray-400 font-normal text-base">({products.length})</span>
        </h1>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">Ajoutez des produits en cliquant sur le cœur.</p>
            ...
            {products.map(product => {
              ...
              return (
                <div key={product.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex gap-4">
                  <Link href={`/produit/${product.slug}`} className="relative w-24 h-24 rounded-xl overflow-hidden bg-gray-50 flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <Link href={`/produit/${product.slug}`} className="text-sm font-semibold text-gray-900 hover:text-teal-500 line-clamp-2">{product.name}</Link>
