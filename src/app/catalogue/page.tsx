      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
        {CATEGORIES.map(c => (
          <button key={c.id} onClick={() => setCat(c.id)}
            className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${cat === c.id ? 'bg-cyan-500 text-white' : 'bg-white text-gray-700 hover:bg-cyan-50 border border-gray-200'}`}>
            {c.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="hidden lg:block w-60 flex-shrink-0">
          <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-6 sticky top-24">
            <div>
              ...
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Nom, marque..." className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
            </div>
            <div>
              <h3 className="font-semibold text-gray-900 mb-3 text-sm">Prix max: {maxPrice >= 2000000 ? 'Tous' : formatPrice(maxPrice)}</h3>
              <input type="range" min={50000} max={2000000} step={50000} value={maxPrice} onChange={e => setMaxPrice(Number(e.target.value))} className="w-full accent-cyan-500" />
              ...
              <div className="space-y-2">
                {allBrands.map(b => (
                  <label key={b} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={selectedBrands.includes(b)} onChange={() => toggleBrand(b)} className="accent-cyan-500" />
            ...
            {(selectedBrands.length > 0 || maxPrice < 2000000 || q) && (
              <button onClick={() => { setSelectedBrands([]); setMaxPrice(2000000); setQ(''); }} className="w-full text-sm text-cyan-500 hover:text-cyan-600 font-medium py-2 border border-cyan-200 rounded-xl">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-4 gap-3">
            ...
            <select value={sort} onChange={e => setSort(e.target.value)} className="ml-auto px-3 py-2 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-cyan-500">
          ...
          {showFilters && (
            <div className="lg:hidden bg-white rounded-2xl border border-gray-100 p-5 mb-4 space-y-4">
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Rechercher..." className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" />
              <div>
              ...
              <div className="flex flex-wrap gap-2">
                {allBrands.map(b => (
                  <button key={b} onClick={() => toggleBrand(b)} className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${selectedBrands.includes(b) ? 'bg-cyan-500 text-white border-cyan-500' : 'bg-white text-gray-700 border-gray-200'}`}>
                    {b}
                  </button>
                ))}
              </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              ...
              <button onClick={() => { setCat('all'); setQ(''); setSelectedBrands([]); setMaxPrice(2000000); }} className="bg-cyan-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-cyan-600 transition-colors">
