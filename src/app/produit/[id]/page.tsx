  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6 flex-wrap">
        <Link href="/" className="hover:text-cyan-500">Accueil</Link>
        <span>/</span>
        <Link href="/catalogue" className="hover:text-cyan-500">Catalogue</Link>
        <span>/</span>
        <Link href={`/catalogue?cat=${product.category}`} className="hover:text-cyan-500 capitalize">{product.category}</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-14">
          ...
          <div className="relative h-72 sm:h-[420px] rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-sm mb-3">
            ...
            {product.badge && (
              <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-bold text-white ${
                ...
                product.badge === 'Populaire' ? 'bg-cyan-500' : 'bg-violet-500'
              }`}>
                {product.badge}
                {discount > 0 && ` -${discount}%`}
              </span>
            ...
            <div className="flex gap-2">
              {product.images.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${activeImg === i ? 'border-cyan-500' : 'border-gray-200 hover:border-gray-400'}`}>
        ...
        {/* Info */}
        <div>
          <p className="text-sm font-semibold text-cyan-500 uppercase tracking-wide mb-1">{product.brand}</p>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${
                ...
                'bg-cyan-500 hover:bg-cyan-600 text-white shadow-lg shadow-cyan-500/25 active:scale-95'
              }`}
            >
        ...
        <div className="flex border-b border-gray-200 bg-gray-50">
          {(['desc','specs','reviews'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3.5 text-sm font-semibold transition-colors ${activeTab === tab ? 'bg-white text-cyan-500 border-b-2 border-cyan-500' : 'text-gray-500 hover:text-gray-900'}`}>
                  ...
                  <ul className="space-y-2">
                    {product.features.map(f => (
                      <li key={f} className="flex items-center gap-2 text-sm text-gray-600">
                        <span className="w-5 h-5 rounded-full bg-cyan-100 text-cyan-500 flex items-center justify-center text-xs font-bold flex-shrink-0">✓</span>
                        {f}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {related.map(p => (
              <Link key={p.id} href={`/produit/${p.id}`} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                ...
                <div className="p-3">
                  <p className="text-xs text-cyan-500 font-semibold">{p.brand}</p>
