      ...
      <div className="border-b border-gray-100 dark:border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm text-gray-400 flex-wrap">
            ...
            <span className="text-gray-700 dark:text-slate-300 truncate max-w-[200px] font-medium">{product.name}</span>
          </nav>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
          ...
          <div className="space-y-3">
            <div className="relative rounded-3xl overflow-hidden bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700" style={{height: '460px'}}>
              <Image
                  ...
                  <button key={i} onClick={() => setActiveImg(i)}
                    className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${
                      activeImg === i
                        ...
                        : 'border-gray-200 dark:border-slate-600 hover:border-teal-300 opacity-70 hover:opacity-100'
                    }`}
                  >
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-slate-100 leading-tight mb-3">{product.name}</h1>

            <div className="flex items-center gap-3 mb-5 pb-5 border-b border-gray-100 dark:border-slate-700">
              <div className="flex gap-0.5">
                {[1,2,3,4,5].map(s => (
                  <svg key={s} className={`w-4 h-4 ${s <= Math.round(product.rating) ? 'text-yellow-400' : 'text-gray-200 dark:text-slate-700'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                  </svg>
                ))}
              </div>
              <span className="text-sm font-bold text-gray-700 dark:text-slate-200">{product.rating}</span>
            ...
            <div className="mb-5">
              <div className="flex items-baseline gap-3">
                <span className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-slate-100">{formatPrice(product.price)}</span>
            ...
            {product.shortDesc && (
              <p className="text-gray-600 dark:text-slate-300 text-sm leading-relaxed mb-5">{product.shortDesc}</p>
            )}

            <ul className="grid grid-cols-1 gap-2 mb-6">
              {product.features.slice(0, 4).map(f => (
                <li key={f} className="flex items-center gap-2 text-sm text-gray-700 dark:text-slate-200">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center rounded-xl overflow-hidden border border-gray-200 dark:border-slate-600">
                <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-11 h-12 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-200 text-xl font-light transition-colors">−</button>
                <span className="w-10 text-center font-bold text-gray-900 dark:text-slate-100 text-sm">{qty}</span>
                <button onClick={() => setQty(q => Math.min(product.stock, q + 1))} className="w-11 h-12 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-200 text-xl font-light transition-colors">+</button>
              </div>
              ...
              <button
                ...
                className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center transition-all ${
                  inWishlist ? 'border-red-400 bg-red-50 dark:bg-red-900/20 text-red-500' : 'border-gray-200 dark:border-slate-600 text-gray-400 hover:border-red-300 hover:text-red-400'
                }`}
              >
            ...
            <div className="grid grid-cols-3 gap-2 mt-4 pt-5 border-t border-gray-100 dark:border-slate-700">
              {[['🔒','Paiement','sécurisé'],['🚚','Livraison','partout en Guinée'],['↩️','Retours','30 jours']].map(([icon,t1,t2]) => (
                <div key={t1} className="flex flex-col items-center text-center gap-1 p-3 rounded-xl bg-gray-50 dark:bg-slate-800">
                  <span className="text-2xl">{icon}</span>
                  <p className="text-xs font-semibold text-gray-700 dark:text-slate-200">{t1}</p>
        ...
        {/* ── TABS ── */}
        <div className="mb-16">
          <div className="flex gap-1 border-b border-gray-200 dark:border-slate-600 mb-6">
          ...
          <div className="bg-gray-50 dark:bg-slate-800 rounded-2xl p-6">
            {activeTab === 'desc' && (
              <div>
                <p className="text-gray-700 dark:text-slate-300 leading-relaxed mb-6">{product.description}</p>
                {product.features.length > 0 && (
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-slate-100 mb-4 text-base">Points forts</h3>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {product.features.map(f => (
                        <li key={f} className="flex items-center gap-2.5 text-sm text-gray-700 dark:text-slate-200 bg-white dark:bg-slate-700 rounded-xl px-4 py-2.5 shadow-sm border border-gray-100 dark:border-slate-600">
                  ...
                  <table className="w-full text-sm">
                    <tbody>
                      {Object.entries(product.specs).map(([key, val], i) => (
                        <tr key={key} className={i % 2 === 0 ? 'bg-white dark:bg-slate-700' : 'bg-gray-50/50 dark:bg-slate-800/50'}>
                          <td className="py-3 px-4 font-semibold text-gray-700 dark:text-slate-200 w-2/5 rounded-l-lg">{key}</td>
                          <td className="py-3 px-4 text-gray-600 dark:text-slate-300 rounded-r-lg">{String(val)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {related.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-extrabold text-gray-900 dark:text-slate-100">Produits similaires</h2>
              {product.category && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-gray-100 dark:border-slate-700 shadow-sm">
        <h4 className="font-bold text-gray-900 dark:text-slate-100 mb-4">Laisser un avis</h4>
        <div className="flex gap-1 mb-3">
          {[1,2,3,4,5].map(s => (
            <button key={s} onClick={() => setRating(s)}>
              <svg className={`w-7 h-7 transition-colors ${s <= rating ? 'text-yellow-400' : 'text-gray-200 dark:text-slate-700 hover:text-yellow-300'}`} fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
              </svg>
            </button>
          ))}
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
        />
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map(r => {
            ...
            return (
              <div key={r.id} className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-gray-100 dark:border-slate-700 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-0.5">
                    {[1,2,3,4,5].map(s => (
                      <svg key={s} className={`w-3.5 h-3.5 ${s <= r.rating ? 'text-yellow-400' : 'text-gray-200 dark:text-slate-700'}`} fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                      </svg>
                    ))}
                  </div>
                </div>
                {r.comment && <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed">{r.comment}</p>}
              </div>
            );
          })}
        </div>
