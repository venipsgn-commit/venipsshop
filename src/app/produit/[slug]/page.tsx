            ...
            <div className="grid grid-cols-3 gap-2 mt-4 pt-5 border-t border-gray-100">
              {[['🔒','Paiement','sécurisé'],['🚚','Livraison','partout en Guinée'],['↩️','Retours','30 jours']].map(([icon,t1,t2]) => (
                <div key={t1} className="flex flex-col items-center text-center gap-1 p-3 rounded-xl bg-gray-50">
                  ...
                  <p className="text-xs font-semibold text-gray-700">{t1}</p>
            ...
            {activeTab === 'desc' && (
              <div>
                ...
                {product.features.length > 0 && (
                  <div>
                    <h3 className="font-bold text-gray-900 mb-4 text-base">Points forts</h3>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {product.features.map(f => (
                        <li key={f} className="flex items-center gap-2.5 text-sm text-gray-700 bg-white rounded-xl px-4 py-2.5 shadow-sm border border-gray-100">
