...
export default function Navbar() {
  ...
  return (
    <header className="bg-[#080b3b] text-white sticky top-0 z-50 shadow-lg">
      {/* Top bar */}
      <div className="border-b border-[#0d1266] hidden sm:block">
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {/* Wishlist */}
          <Link href="/compte/wishlist" className="relative p-2 hover:bg-[#0d1266] rounded-lg transition-colors hidden sm:flex">
          ...
          {user ? (
            <div ref={dropRef} className="relative">
              <button onClick={() => setDropOpen(!dropOpen)} className="flex items-center gap-2 p-2 hover:bg-[#0d1266] rounded-lg transition-colors">
            ...
            <Link href="/auth/connexion" className="hidden sm:flex items-center gap-2 px-3 py-2 hover:bg-[#0d1266] rounded-lg transition-colors text-sm">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Cart */}
          <button onClick={toggle} className="relative p-2 hover:bg-[#0d1266] rounded-lg transition-colors">
          ...
          {/* Mobile menu */}
          <button onClick={() => setMenuOpen(!menuOpen)} className="sm:hidden p-2 hover:bg-[#0d1266] rounded-lg transition-colors">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              {menuOpen
      ...
      {/* Category nav */}
      <div className="hidden sm:block bg-[#0d1266] border-t border-[#1a2080]">
        <div className="max-w-7xl mx-auto px-4 flex gap-1 h-10 items-center overflow-x-auto">
          {NAV.map(item => (
            <Link key={item.href} href={item.href} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-[#1a2080] transition-colors whitespace-nowrap">
      ...
      {/* Mobile menu panel */}
      {menuOpen && (
        <div className="sm:hidden bg-[#0d1266] border-t border-[#1a2080] py-2">
          {NAV.map(item => (
            <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-gray-300 hover:text-white hover:bg-[#1a2080] transition-colors">
              <span>{item.icon}</span>{item.label}
            </Link>
          ))}
          <div className="border-t border-[#1a2080] mt-2 pt-2">
            {user ? (
              <>
                <Link href="/compte" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-gray-300 hover:text-white hover:bg-[#1a2080] transition-colors">
                  👤 Mon compte
                </Link>
                <button onClick={() => { logout(); setMenuOpen(false); }} className="w-full flex items-center gap-3 px-5 py-3 text-sm text-red-400 hover:bg-[#1a2080] transition-colors">
                  🚪 Déconnexion
                </button>
              </>
            ) : (
              <Link href="/auth/connexion" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-gray-300 hover:text-white hover:bg-[#1a2080] transition-colors">
