...
export default function ProductCard({ product }: { product: AnyProduct }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col overflow-hidden">

      {/* Image */}
      <Link href={href} className="relative block overflow-hidden bg-gray-50 dark:bg-slate-700">
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1">
        ...
        <Link href={href}>
          <h3 className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-slate-100 hover:text-teal-600 dark:hover:text-teal-400 transition-colors line-clamp-2 leading-snug mb-2">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          <div className="flex">
            {[1,2,3,4,5].map(s => (
              <svg key={s} className={`w-3 h-3 ${s <= Math.round(product.rating) ? 'text-yellow-400' : 'text-gray-200 dark:text-slate-700'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="mt-auto">
          <div className="flex items-end gap-2 mb-3">
            <span className="text-base sm:text-lg font-extrabold text-gray-900 dark:text-slate-100">{formatPrice(product.price)}</span>
